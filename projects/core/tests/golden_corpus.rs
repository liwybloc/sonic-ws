...
fn packet(kind: PacketType, count: usize) -> PacketDef {
    PacketDef {
        ...
        schema: PacketSchema {
            ...
            data_min: SchemaLimit::Single(count as u64),
            data_max: SchemaLimit::Single(count as u64),
            data_batching: 0,
            max_batch_size: 0,
            dont_spread: false,
            auto_flatten: false,
...
fn kind(name: &str) -> PacketType {
    match name {
        "BYTES" => PacketType::Bytes,
        "UBYTES" => PacketType::UBytes,
        "SHORTS" => PacketType::Shorts,
        "USHORTS" => PacketType::UShorts,
        "VARINT" => PacketType::VarInt,
        "UVARINT" => PacketType::UVarInt,
        "DELTAS" => PacketType::Deltas,
        "STRINGS_ASCII" => PacketType::StringsAscii,
        "STRINGS_UTF16" => PacketType::StringsUtf16,
        "BOOLEANS" => PacketType::Booleans,
        "HEX" => PacketType::Hex,
...
fn shared_golden_vectors_encode_and_decode() {
        ...
        .filter(|line| line.contains("{ \"name\":") && !line.contains("\"schema\""))
    {
        ...
        let parts: Vec<&str> = if raw_values.is_empty() {
            vec![]
        } else {
            raw_values.split(", ").collect()
        };
        let value = if kind == PacketType::Hex {
            ...
            SonicValue::Array(
                parts
                    .iter()
                    .map(|raw| match kind {
                        PacketType::StringsAscii | PacketType::StringsUtf16 => {
                            SonicValue::String(raw.trim_matches('"').into())
                        }
                        PacketType::Booleans => SonicValue::Bool(*raw == "true"),
                        PacketType::UBytes | PacketType::UShorts | PacketType::UVarInt => {
                            SonicValue::U64(raw.parse().unwrap())
                        }
        ...
        let count = if kind == PacketType::Hex {
            expected.len()
        } else {
            parts.len()
        };
        ...
        assert_eq!(
            encode_packet(&definition, &value).unwrap(),
            expected,
            "{name}"
        );
        assert_eq!(
            decode_packet(&definition, &expected).unwrap(),
            value,
            "{name}"
        );
    }
