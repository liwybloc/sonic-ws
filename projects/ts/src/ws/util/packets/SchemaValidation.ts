...
export function ValidatePacketSchema(
    ...
    if (packets.length > 254) errors.push(`Packet table contains ${packets.length} packets; the maximum is 254`);

    for (const packet of packets) {
        if (tags.has(packet.tag)) errors.push(`Duplicate packet tag "${packet.tag}"`);
        tags.add(packet.tag);
        if (packet.isParent) parents.set(packet.tag, packet);
        if (packet.replay && packet.dataBatching) errors.push(`Packet "${packet.tag}" combines replay with batching`);
        ...
        if (packet.fields && !packet.autoFlatten && !packet.object && packet.dataMin === packet.dataMax && packet.fields.length !== packet.dataMax)
            errors.push(`Packet "${packet.tag}" schema length does not match its fixed value count`);
        if (options.warnUnbounded && options.direction === "client" && (Array.isArray(packet.dataMax)
        ...
        if (packet.parent && packet.variant) {
            ...
            if (variants.has(key)) errors.push(`Duplicate packet-group variant "${key}"`);
            variants.add(key);
        }
    }
    ...
    for (const packet of packets) {
        ...
        if (!parent) errors.push(`Packet "${packet.tag}" references missing group parent "${packet.parent}"`);
        else if (parent.type !== PacketType.NONE) errors.push(`Packet-group parent "${packet.parent}" must use PacketType.NONE`);
    }

    return { errors, warnings };
}

export function AssertPacketSchema(packets: readonly Packet<any>[], options?: Parameters<typeof ValidatePacketSchema>[1]): void {
    const result = ValidatePacketSchema(packets, options);
    if (result.errors.length) throw new Error(`Invalid SonicWS packet schema:\n- ${result.errors.join("\n- ")}`);
    for (const warning of result.warnings) console.warn(`SonicWS schema warning: ${warning}`);
}
