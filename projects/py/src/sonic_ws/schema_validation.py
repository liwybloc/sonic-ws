...
def validate_packet_schema(packets, *, direction=None, warn_unbounded=False):
    ...
    if len(packets) > 254:
        errors.append(
            f"packet table contains {len(packets)} packets; the maximum is 254"
        )
    ...
    for packet in packets:
        ...
        if (
            packet.value_min is not None
            and packet.value_min < 0
            and any(kind in UNSIGNED for kind in packet.types)
        ):
            errors.append(
                f'packet "{packet.tag}" has a negative minimum for an unsigned type'
            )
        if (
            warn_unbounded
            and direction == "client"
            and any(value >= 2_048_383 for value in packet.data_maxes)
        ):
            warnings.append(
                f'client packet "{packet.tag}" has an effectively unbounded value count'
            )
    ...
    for packet in packets:
        ...
        if parent is None:
            errors.append(
                f'packet "{packet.tag}" references missing group parent "{packet.parent}"'
            )
        elif parent.type != PacketType.NONE:
            errors.append(
                f'packet-group parent "{packet.parent}" must use PacketType.NONE'
            )
