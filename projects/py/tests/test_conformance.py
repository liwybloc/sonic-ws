    ...
    for vector in corpus["vectors"]:
        ...
        count = (
            ...
            if vector["type"] == "HEX"
                ...
                else len(vector.get("schema", ())) or 1
            )
        )
        settings = {
            "tag": vector["name"],
            "type": getattr(PacketType, vector["type"]),
            "schema": vector.get("schema"),
            "autoFlatten": vector.get("autoFlatten", False),
            "quantized": vector.get("quantized"),
        }
        if not vector.get("autoFlatten"):
            settings.update(
                dataMin=1 if vector["type"] == "HEX" else count, dataMax=count
            )
        ...
        assert packet.decode(encoded) == (
            values[0] if vector["type"] == "HEX" else values
        ), vector["name"]
