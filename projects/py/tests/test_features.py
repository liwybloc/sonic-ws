...
class FeatureTests(unittest.TestCase):
    ...
    def test_schema_object_matches_positional_wire(self):
        ...
        self.assertEqual(packet.decode(packet.encode((1, 2, 3))), {"dx": 1, "dy": 2, "dz": 3})
    ...
    def test_row_major_auto_flatten(self):
        packet = CreatePacket(tag="rows", type=PacketType.VARINT, schema=["id", "x"], autoFlatten=True)
    ...
    def test_column_major_auto_transpose(self):
        packet = CreateObjPacket(tag="columns", types=[PacketType.VARINT, PacketType.STRINGS_ASCII], schema=["x", "label"], autoTranspose=True, noDataRange=True)
        rows = [{"x": 1, "label": "one"}, {"x": 2, "label": "two"}]
        self.assertEqual(packet.prepare_send((rows,)), [[1, 2], ["one", "two"]])
        self.assertEqual(packet.decode(packet.encode((rows,))), rows)
        legacy = CreateObjPacket(tag="legacy", types=[PacketType.VARINT, PacketType.VARINT], autoFlatten=True, noDataRange=True)
        self.assertEqual(legacy.decode(legacy.encode(([[1, 2], [3, 4]],))), [[1, 2], [3, 4]])

    def test_quantization_and_bounds(self):
        packet = CreatePacket(tag="q", type=PacketType.SHORTS, schema=["x", "y"], dataMin=2, dataMax=2, quantized={"scale": 100, "trackError": True}, min=-1, max=1)
        self.assertEqual(packet.prepare_send(({"x": .5, "y": -.5},)), [50, -50])
        self.assertEqual(packet.decode(packet.encode(({"x": .5, "y": -.5},))), {"x": .5, "y": -.5})
    ...
    def test_quantization_error_feedback(self):
        packet = CreatePacket(tag="feedback", type=PacketType.VARINT, quantized={"scale": 1024})
        ...
        self.assertLessEqual(abs(total / 1024 - 1528.3), .5 / 1024)
        stateless = CreatePacket(tag="stateless", type=PacketType.VARINT, quantized={"scale": 1024, "trackError": False})
