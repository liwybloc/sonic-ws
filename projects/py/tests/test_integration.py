...
async def main():
    ...
    server = SonicWSServer(
        client_packets=packets, server_packets=packets, port=0, adapter=Adapter()
    )
    server_values = []
    ...
    client.on(
        "point",
        lambda value: room_value.set_result(value) if not room_value.done() else None,
    )
    await server.broadcast_room("world:one", "point", {"x": 8, "y": 9})
    assert await asyncio.wait_for(room_value, 2) == {"x": 8, "y": 9}
    assert any(
        event[0] == "join" and event[2] == "world:one" for event in adapter_events
    )
    assert any(event[0] == "publish" for event in adapter_events)

    recovered = asyncio.get_running_loop().create_future()
    client.on_recovered(
        lambda event: recovered.set_result(event) if not recovered.done() else None
    )
    replayed = asyncio.get_running_loop().create_future()
    client.on(
        "point",
        lambda value: (
            replayed.set_result(value)
            if value.get("x") == 10 and not replayed.done()
            else None
        ),
    )
    ...
    while server.connections:
        await asyncio.sleep(0.01)
    missed = server.server_packets.packet("point").encode(
        ({"x": 10, "y": 11},), connection.id
    )
    server.replay_frame(
        connection, bytes([server.server_packets.code("point")]) + missed
    )
    ...
    client.set_backpressure_limits(
        volatile_at_bytes=1024 * 1024, close_at_bytes=16 * 1024 * 1024
    )
